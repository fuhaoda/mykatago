...
class Board:
    ...
    def __init__(self, size, copy_other=None):
        ...
        self.dy = size + 1 #the difference loc value between each row
        ...
        if copy_other is not None:
            ...
            self.board = np.zeros(shape=(self.arrsize), dtype=np.int8)  # np.int8 : Byte (-128 to 127)
            self.group_head = np.zeros(shape=(self.arrsize), dtype=np.int16)  # todo: what's this?
            ...
            for i in range(-1, size + 1):  # board valid value is from 0 to size-1 with length equal to size.
    ...
    @staticmethod
    def get_opp(pla):  # black = 1 and white = 2
        return 3 - pla
    ...
    def is_adjacent(self, loc1, loc2):
        return loc1 == loc2 + self.adj[0] or loc1 == loc2 + self.adj[1] or loc1 == loc2 + self.adj[2] or loc1 == loc2 + \
               self.adj[3]
    ...
    def is_simple_eye(self, pla, loc):
        ...
        if (self.board[adj0] != pla and self.board[adj0] != Board.WALL) or \
            ...
            return False
        #if for any position connected with either other player or empty position, then that is not an eye.
        ...
        #<1>: if more than two of the corners are opp's stones, it is not an eye
        #<2>: if none of them are opp stones, it is an eye. todo: this may not be a real eye in flower shape
    ...
    #<1>: against wall with 1 opp in the corner.
    #<2>: not against wall but with 1 opp in the corner.

    def would_be_legal(self, pla, loc):
        if pla != Board.BLACK and pla != Board.WHITE:
            return False
        if loc == Board.PASS_LOC: # PASS_LOC = 0 todo did we code pass as one of board position?
            return True
        if not self.is_on_board(loc):
            return False
        if self.board[loc] != Board.EMPTY:
            return False
        if self.would_be_single_stone_suicide(pla, loc): #<1>
            return False
        if loc == self.simple_ko_point: #<2>
            return False
        return True #todo: does it allow regular suicide?

    #<1>: single stone suicide is not allowed
    #<2>: ko is not allowed
    ...
    def would_be_suicide(self, pla, loc):
        ...
        if self.board[adj0] == Board.EMPTY or (
                self.board[adj0] == opp and self.group_liberty_count[self.group_head[adj0]] == 1) or \
                self.board[adj1] == Board.EMPTY or (
                self.board[adj1] == opp and self.group_liberty_count[self.group_head[adj1]] == 1) or \
                self.board[adj2] == Board.EMPTY or (
                self.board[adj2] == opp and self.group_liberty_count[self.group_head[adj2]] == 1) or \
                self.board[adj3] == Board.EMPTY or (
                self.board[adj3] == opp and self.group_liberty_count[self.group_head[adj3]] == 1):
            return False
    ...
    def get_liberties_after_play(self, pla, loc, maxLibs): #todo: need to understand the logic here
        opp = Board.get_opp(pla)
        libs = []
        capturedGroupHeads = []
        ...
        for i in range(4):
            ...
            if self.board[adj] == Board.EMPTY:
                libs.append(adj)
                if len(libs) >= maxLibs: #nice idea that we don't need to count all the liberties.
                    return maxLibs
    ...
    def to_string(self):
        def get_piece(x, y):
            ...
            if self.board[loc] == Board.BLACK:
                ...
                return 'O '
            elif (x == 3 or x == self.size / 2 or x == self.size - 1 - 3) and ( #this lines are ugly!)
                    y == 3 or y == self.size / 2 or y == self.size - 1 - 3):
                return '* '
            else:
                return '. '
        ...
        def get_piece(x, y):
            ...
            if self.board[loc] == Board.BLACK or self.board[loc] == Board.WHITE:
                ...
                if libs <= 9:
                    ...
                    return '@ '
            elif (x == 3 or x == self.size / 2 or x == self.size - 1 - 3) and (
                    y == 3 or y == self.size / 2 or y == self.size - 1 - 3):
                return '* '
            else:
                return '. '
    ...
    def playRecordedUnsafe(self, pla, loc):
        ...
        # Suicide
        selfCap = False
        if self.board[loc] == Board.EMPTY: #todo: why?
            selfCap = True
    ...
    def add_unsafe(self, pla, loc):
        ...
        # <1> count the liberties for the stone on the new location.
        # <2> remove a liberty for the groups which are in adj positions.
        ...
        # for suicide, it add then remove loc
    ...
    def merge_unsafe(self, loc0, loc1):
        if self.group_stone_count[self.group_head[loc0]] >= self.group_stone_count[self.group_head[loc1]]: #<1>
            parent = loc0
            child = loc1
        else:
            child = loc0
        ...
        #<1> smaller group is the child group to reduce the work.

        phead = self.group_head[parent]
        chead = self.group_head[child]
        if phead == chead: #after one merge, the next neighbor can already be connected.
            return
        ...
        self.group_next[clast] = phead #<1>
        self.group_next[plast] = chead
        self.group_prev[chead] = plast #<2>
        self.group_prev[phead] = clast

        #<1> open the chain to make the last item in paranent list to point to the first item of the child,
        # and last item in child to point to the paranent head.
        #<2> this is a reverse chain

    # Remove all stones in a group
    def remove_unsafe(self, group): # the input group is a stone belong to a group
    ...
    def searchIsLadderCaptured(self, loc, defenderFirst):
        ...
        if self.group_liberty_count[self.group_head[loc]] > 2 or (
                defenderFirst and self.group_liberty_count[self.group_head[loc]] > 1):
            return False
        ...
        moveListCur = [0 for i in
                       range(arrSize)]  # Current move list idx searched, equal to -1 if list has not been generated.
        ...
        while True:
            ...
            if moveListCur[stackIdx] == -1:
                ...
                if isDefender:
                    ...
                    if libs0 == 0 and libs1 == 0 and self.wouldBeKoCapture(move0, opp) and self.wouldBeKoCapture(move1,
                                                                                                                 opp):
                        if self.get_liberties_after_play(pla, move0, 3) <= 2 and self.get_liberties_after_play(pla,
                                                                                                               move1,
                                                                                                               3) <= 2:
    ...
    def calculateArea(self, result, nonPassAliveStones, safeBigTerritories, unsafeBigTerritories,
                      isMultiStoneSuicideLegal):
        for i in range(self.arrsize):
            result[i] = Board.EMPTY
        self.calculateAreaForPla(Board.BLACK, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal,
                                 result)
        self.calculateAreaForPla(Board.WHITE, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal,
                                 result)
    ...
    def calculateAreaForPla(self, pla, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal, result):
        ...
        for i in range(numRegions):
            ...
            shouldMark = shouldMark or (safeBigTerritories and atLeastOnePla and not containsOpp[i] and not
            bordersNonPassAlivePlaByHead[head])
